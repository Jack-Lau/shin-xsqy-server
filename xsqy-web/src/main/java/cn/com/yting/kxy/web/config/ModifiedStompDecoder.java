 ...
 * @since 4.0
 */
public class ModifiedStompDecoder extends StompDecoder {

    private static final Logger logger = LoggerFactory.getLogger(ModifiedStompDecoder.class);

    public static final byte END_OF_MESSAGE = 0x03;

    static final byte[] HEARTBEAT_PAYLOAD = new byte[] {'\n'};

    private static final Method updateSimpMessageHeadersFromStompHeaders;

    static {
        try {
            updateSimpMessageHeadersFromStompHeaders = StompHeaderAccessor.class.getDeclaredMethod("updateSimpMessageHeadersFromStompHeaders");
            updateSimpMessageHeadersFromStompHeaders.setAccessible(true);
        } catch (Exception ex) {
    ...
    private Message<byte[]> decodeMessage(ByteBuffer byteBuffer, @Nullable MultiValueMap<String, String> headers) {
        ...
        if (command.length() > 0) {
            ...
            if (payload != null) {
                ...
                updateSimpMessageHeadersFromStompHeaders(headerAccessor);
    ...
    private byte[] readPayload(ByteBuffer byteBuffer, StompHeaderAccessor headerAccessor) {
        ...
        if (contentLength != null && contentLength >= 0) {
            if (byteBuffer.remaining() > contentLength) {
                ...
                if (byteBuffer.get() != END_OF_MESSAGE) {
                    throw new StompConversionException("Frame must be terminated with a 0x03");
                }
                return payload;
            }
            else {
    ...
    void updateSimpMessageHeadersFromStompHeaders(StompHeaderAccessor accessor) {
        try {
            updateSimpMessageHeadersFromStompHeaders.invoke(accessor);
