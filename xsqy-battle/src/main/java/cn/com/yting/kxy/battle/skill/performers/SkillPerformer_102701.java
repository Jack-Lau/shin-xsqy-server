 ...
 * @author Darkholme
 */
public class SkillPerformer_102701 implements BuffAttachSkillPerformer {
    ...
    public List<Affect> processMainTarget(Unit source, Unit target, int processCount, SkillParameterTable skillParameterTable, int skillLevel, int targetCount, DamageValue cost) {
        ...
        double extra易疗率 = source.getParameter(ParameterNameConstants.大慈心光渡强化).getValue() > 0 && processCount > 1 ? -0.75 : 0;
        SkillParameterTable spt = SkillParameterTable.builder()
                ...
                .build();
        affects.add(new RecoverAffect(true, source, spt, skillLevel, targetCount, processCount));
        //
        if (source.getParameter(ParameterNameConstants.普陀奇穴6_甲).getValue() > 0) {
            int countDown = 2;
            SimpleParameterSpace parameterSpace = new SimpleParameterSpace();
            parameterSpace.addParameterBase(ParameterNameConstants.易伤率,
                    new SimpleParameterBase(-0.05 + source.getParameter(ParameterNameConstants.普陀奇穴6_甲).getValue() * -0.005));
            //
            BuffPrototype prototype = buffRef.get().getPrototype();
            Buff buff = prototype.createBuff(source, 102701, parameterSpace, countDown, skillLevel);
            //
