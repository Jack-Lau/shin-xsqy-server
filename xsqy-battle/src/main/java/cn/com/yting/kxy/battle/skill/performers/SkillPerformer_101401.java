 ...
 * @author Darkholme
 */
public class SkillPerformer_101401 implements ResourceSkillPerformer {
    ...
    public List<Affect> processMainTarget(Unit source, Unit target, int processCount, SkillParameterTable skillParameterTable, int skillLevel, int targetCount, DamageValue cost) {
        double extra暴击率 = source.getParameter(ParameterNameConstants.鬼神泣强化).getValue() > 0 ? 0.1 : 0;
        double extra易伤率 = source.getParameter(ParameterNameConstants.凌霄奇穴7_甲).getValue() > 0 ? -0.35 : 0;
