 ...
 * @author Darkholme
 */
public class SkillPerformer_104701 implements ResourceSkillPerformer {
    ...
    public List<Affect> processMainTarget(Unit source, Unit target, int processCount, SkillParameterTable skillParameterTable, int skillLevel, int targetCount, DamageValue cost) {
        double extra易伤率 = 0;
        if (source.getParameter(ParameterNameConstants.八方湮灭强化).getValue() > 0) {
            extra易伤率 = 0.1;
        }
        if (source.getParameter(ParameterNameConstants.五庄奇穴7_甲).getValue() > 0) {
            double baseSpCost = 70 * source.getParameter(ParameterNameConstants.怒气消耗率).getValue();
            if (cost.getSp() > baseSpCost) {
                extra易伤率 += (cost.getSp() - baseSpCost) * (0.005 + source.getParameter(ParameterNameConstants.五庄奇穴7_甲).getValue() * 0.00025);
            }
        }
