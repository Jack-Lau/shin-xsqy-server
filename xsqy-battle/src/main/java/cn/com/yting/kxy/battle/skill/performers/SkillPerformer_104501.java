 ...
 * @author Darkholme
 */
public class SkillPerformer_104501 implements ResourceSkillPerformer {
    ...
    public List<Affect> processMainTarget(Unit source, Unit target, int processCount, SkillParameterTable skillParameterTable, int skillLevel, int targetCount, DamageValue cost) {
        double extra易伤率 = source.getParameter(ParameterNameConstants.霜天剑舞强化).getValue() > 0 ? 0.075 : 0;
        double extra法伤 = source.getParameter(ParameterNameConstants.五庄奇穴4_乙).getValue() > 0
                ? (target.getHp().getUpperLimit().getValue() - target.getHp().getValue()) * (0.01 + source.getParameter(ParameterNameConstants.五庄奇穴4_乙).getValue() * 0.00075) : 0;
