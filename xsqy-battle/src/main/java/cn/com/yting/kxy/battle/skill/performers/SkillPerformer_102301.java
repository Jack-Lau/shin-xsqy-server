 ...
 * @author Darkholme
 */
public class SkillPerformer_102301 implements ResourceSkillPerformer {
    ...
    public List<Affect> processMainTarget(Unit source, Unit target, int processCount, SkillParameterTable skillParameterTable, int skillLevel, int targetCount, DamageValue cost) {
        double extra易疗率 = source.getParameter(ParameterNameConstants.回风饮露强化).getValue() > 0 ? 0.15 : 0;
        double extra法伤 = source.getParameter(ParameterNameConstants.普陀奇穴4_乙).getValue() > 0
                ? source.getHp().getUpperLimit().getValue() * (0.01 + source.getParameter(ParameterNameConstants.普陀奇穴4_乙).getValue() * 0.002) : 0;
        ...
        return Collections.singletonList(
                new RecoverAffect(false, source, spt, skillLevel, targetCount, processCount)
        );
    }
