 ...
 * @author Darkholme
 */
public class SkillPerformer_102401 implements ResourceSkillPerformer {
    ...
    public List<Affect> processMainTarget(Unit source, Unit target, int processCount, SkillParameterTable skillParameterTable, int skillLevel, int targetCount, DamageValue cost) {
        double extra防御穿透率 = source.getParameter(ParameterNameConstants.云海潮生强化).getValue() > 0 ? 0.075 : 0;
        double extra易伤率 = source.getParameter(ParameterNameConstants.普陀奇穴3_甲).getValue() > 0
                ? 0.05 + source.getParameter(ParameterNameConstants.普陀奇穴3_甲).getValue() * 0.005 : 0;
        extra防御穿透率 += source.getParameter(ParameterNameConstants.普陀奇穴3_乙).getValue() > 0
                ? 0.05 + source.getParameter(ParameterNameConstants.普陀奇穴3_乙).getValue() * 0.004 : 0;
